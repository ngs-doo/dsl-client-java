...
class HttpCrudProxy implements CrudProxy {
    ...
    public <T extends Identifiable> Future<T> read(
        ...
        return client.sendRequest(JsonSerialization.buildType(manifest),
                CRUD_URI + domainName + "?uri=" + encode(uri), "GET", null,
                new int[] { 200 });
    }
    ...
    public <TAggregate extends AggregateRoot> Future<TAggregate> create(
        ...
        final String domainName = client.getDslName(manifest);
        return client.sendRequest(JsonSerialization.buildType(manifest),
                CRUD_URI + domainName, "POST", aggregate, new int[] { 201 });
    }
    ...
    public <TAggregate extends AggregateRoot> Future<TAggregate> update(
        ...
        final String domainName = client.getDslName(manifest);
        return client.sendRequest(JsonSerialization.buildType(manifest),
                CRUD_URI + domainName + "?uri=" + encode(uri), "PUT",
                aggregate, new int[] { 200 });
    }
    ...
    public <TAggregate extends AggregateRoot> Future<TAggregate> delete(
        ...
        final String domainName = client.getDslName(manifest);
        return client.sendRequest(JsonSerialization.buildType(manifest),
                CRUD_URI + domainName + "?uri=" + encode(uri), "DELETE", null,
                new int[] { 200 });
    }
}
