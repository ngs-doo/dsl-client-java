...
public class JsonSerialization {
	...
	public void serialize(final Element value, final JsonGenerator gen, final SerializerProvider sP)
	    ...
	    /*
	     * The Xml needs to be cleaned from whitespace text nodes, otherwise
	     * the converted document won't match Json.Net's conversion
	     */
	    ...
	    /*
	     * Otherwise write recursively build the JSON hashmap, and write it
	     * to the generator
	     */
	    else {
    ...
    /**
     * Recursively builds a JSON object from the given root element TODO: Needs
     * to serialize empty string nodes as null; partially added, but needs to be
     * checked
     */
	...
	/*
	 * If there are no children, and no attributes, just return the node's
	 * text content value
	 */
	...
	for (final Map.Entry<String, LinkedList<Node>> name_ChildrenHavingName : childrenByName.entrySet()) {
		...
		/*
		 * If the node is an XML element, recursively convert it's tree
		 * to an Object
		 */
     ...
     */
    private static void buildXmlFromHashMap(final Document doc, final Element subtreeRootElement,
	...
	if (elementContent instanceof HashMap) {
		...
		if (childEntry.getKey().startsWith("@")) {
			...
			/*
			 * All other nodes whose name starts with a '#' are
			 * invalid XML nodes, and thus ignored:
			 */
		    }
		} else {
     ...
     *            The parent document
     * @param listHeadNode
     *            The first node of the list, needs to be created before this
     *            method call
    ...
    private static final SimpleModule serializationModule = new SimpleModule("SerializationModule", new Version(0, 0,
	    0, "SNAPSHOT", "com.dslplatform", "dsl-client")).addSerializer(LocalDate.class, dateSerializer)
	    .addSerializer(DateTime.class, timestampSerializer).addSerializer(Element.class, xmlSerializer);

    private static final ObjectMapper serializationMapper = new ObjectMapper().registerModule(serializationModule)
	    .setSerializationInclusion(Include.NON_NULL).disable(SerializationFeature.FAIL_ON_EMPTY_BEANS);
    ...
    private static final SimpleModule deserializationModule = new SimpleModule("DeserializationModule", new Version(0,
	    0, 0, "SNAPSHOT", "com.dslplatform", "dsl-client")).addDeserializer(LocalDate.class, dateDeserializer)
	    .addDeserializer(DateTime.class, timestampDeserializer).addDeserializer(Element.class, xmlDeserializer);
    ...
    public static <T> T deserialize(final JavaType type, final String data, final ServiceLocator locator)
	    throws IOException {
