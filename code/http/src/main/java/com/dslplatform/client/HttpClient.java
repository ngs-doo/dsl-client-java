    ...
    private <TArgument> Response doRawRequest(
        ...
        if (content == null) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("Sending request [{}]: {}, no content", method,
                        service);
            }
        } else {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Sending request [{}]: {}, content size: {} bytes",
                        method, service, jsonBody.length());
            }
        }
        ...
        if (logger.isDebugEnabled()) {
            ...
            logger.debug("Received response [{}, {} bytes] in {} ms",
                    response.code, response.body.length, time);

            if (logger.isTraceEnabled()) {
                logger.trace("Received response body: {}",
    ...
    public <TArgument> Future<byte[]> sendRawRequest(
        ...
        return executorService.submit(new Callable<byte[]>() {
            ...
            public byte[] call() throws IOException {
                return doRawRequest(service, headers, method, content,
                        expected, start).body;
            }
        });
    }
