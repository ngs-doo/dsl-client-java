...
public class Xml2JsonRoundTripTest {
    ...
    public static Collection sourceFiles() throws URISyntaxException {
        final File[] sourceFilesArray =
                getFileForResource("/roundtripTests/xml/source/").listFiles();

        final List<Object[]> sourceFilesList = new ArrayList<Object[]>();
        for (final File f : sourceFilesArray)
            if (f.isFile()) {
    ...
    public void assertRoundTripEquivalenceWithReferenceConversion()
        ...
        System.out
                .println("-----------------------------------------------------");
        System.out.println("| Testiramo za datoteku: "
                + xmlSourceFile.getName());
        System.out
                .println("-----------------------------------------------------");
        ...
        final File referenceFile_json =
                getFileForResource("/roundtripTests/xml/reference/"
                        + convertedFilename_json);
        assertTrue("The reference JSON file does not exist for: "
                + sourceFilename_xml, referenceFile_json != null
        ...
        assertTrue(
                "The reference XML->JSON roundtrip file does not exist for: "
                        + sourceFilename_xml,
                referenceRoundtripFile_xml != null
                        && referenceRoundtripFile_xml.exists());

        /* Parse input files */
        final Document source_xml = parseXmlFile(xmlSourceFile);
        final String referenceJson = stringFromFile(referenceFile_json);
        final Document referenceRoundTrip_xml =
                parseXmlFile(referenceRoundtripFile_xml);
        ...
        final Document roundtripXmlDocument =
                xmlDocumentFromJson(convertedJson);
        ...
        assertXmlEquivalence("The roundtrip XML does not match the source XML",
                roundtripXmlDocument, source_xml);
        assertXmlEquivalence(
                "The reference roundtrip XML does not match the source XML",
                referenceRoundTrip_xml, source_xml);
    }

    private static void assertJsonEquivalence(final String lhs, final String rhs)
    ...
    private static void assertXmlEquivalence(
            final String message,
            final Document lhs,
            final Document rhs) {
        final XmlBruteForceComparator comparator =
                new XmlBruteForceComparator();
